...
wildfires_final.num <- wildfires_final %>% select(where(is.numeric)) %>% scale() %>% as.data.frame()
#Dummies
wildfires_final.dumm <- wildfires_final %>% select(!where(is.numeric)) %>% 
...
#Data SPLITTING
set.seed(346)
index.tr <- createDataPartition(y = wildfires_final.dat$fire_size, p = 0.8, list = FALSE)
...
set.seed(123)
fire.reg <- rpart(fire_size ~ ., data=data.tr)
rpart.plot(fire.reg)
plotcp(fire.reg)
fire.tree.prune <- prune(fire.reg, cp=0.021)
rpart.plot(fire.tree.prune)

## Scoring
RMSE(predict(fire.tree.prune, newdata = data.te), data.te$fire_size)
## 15010.94
MAE(predict(fire.tree.prune, newdata = data.te), data.te$fire_size)
#2947.147
...
grid <- expand.grid(C=c(0.01,0.013,0.023,0.051))
set.seed(346)
fire_reg_tree.cv <- train(fire_size ~., data = data.tr, method = "rpart",
                trControl=trctrl, trace=0, cp=grid)
fire_reg_tree.cv
## Error: All RMSE metric values are missing
